...
export const LevelPickerModal: React.FC<LevelPickerModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-md max-h-[90vh] overflow-hidden flex flex-col shadow-2xl">
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-800">
          <div>
            <h3 className="text-base font-bold text-slate-100">Seleccionar Nivel</h3>
            <p className="text-xs text-slate-400 mt-0.5">
              {participant.nombre} • Récord actual: <strong className="text-amber-400">Nivel {participant.mejorNivel || 1}</strong>
            </p>
          </div>
          <button
            onClick={onClose}
