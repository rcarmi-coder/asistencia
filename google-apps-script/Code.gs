...
function handleSaveAttendance(sheet, body) {
  ...
  let namesData = currentLastRow >= 2 ? sheet.getRange(2, 1, currentLastRow - 1, 1).getValues() : [];
  const nameToRowMap = {};
  ...
  // 2.1 AUTO-AGREGAR ALUMNOS NUEVOS: Si el payload contiene alumnos que no existen en Columna A, agregarlos
  const missingRows = [];
  for (let r = 0; r < records.length; r++) {
    const recName = String(records[r].name || records[r].nombre || "").trim();
    if (recName && !nameToRowMap[recName.toLowerCase()]) {
      const bestLvl = parseNumberOrNull(records[r].bestLevel || records[r].mejorNivel) || "";
      missingRows.push([recName, bestLvl]);
      nameToRowMap[recName.toLowerCase()] = currentLastRow + missingRows.length;
    }
  }

  if (missingRows.length > 0) {
    sheet.getRange(currentLastRow + 1, 1, missingRows.length, 2).setValues(missingRows);
    currentLastRow = sheet.getLastRow();
    namesData = sheet.getRange(2, 1, currentLastRow - 1, 1).getValues();
  }
